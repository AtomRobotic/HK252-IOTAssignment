...
void tiny_ml_task(void *pvParameters)
{
    ...
    setupTinyML();
    SensorData rcvSensorData = app->sensorData;
    while (1)
    {
        if (xQueuePeek(app->xQueueSensor, &rcvSensorData, 0) == pdTRUE)
        {
            ...
            input->data.f[0] = rcvSensorData.temperature;
            input->data.f[1] = rcvSensorData.humidity;
            Serial.println("------------------TinyML Task------------------");
            Serial.println("Input to model - Temperature: ");
